...
# Lists to store person_ids and record_nums
person_ids = []
record_nums = []

for line in lines:
    print("Processing line:", line)  # Debugging: Print the current line being processed
    
    # Use regular expression to extract the person_id and record_num
    match = re.search(r'Person_(\d+)/rec_(\d+)', line)
    
    if match:
        # Extract person_id and record_num from the match object
        person_id = int(match.group(1))  # Convert person_id to integer
        record_num = int(match.group(2))  # Convert record_num to integer
        
        # Append to the respective lists
        person_ids.append(person_id)
        record_nums.append(record_num)

# Print the extracted person_ids and record_nums
print("person_ids:", person_ids)
print("record_nums:", record_nums)